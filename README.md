...
历经多个迭代，Ayabot 首个正式版！新增大量功能与稳定性改进：

- 🎨 **精美送礼卡片导出** — PNG 所见即所得，modern-screenshot 引擎，完美还原毛玻璃/渐变/Flex 布局
- 👥 **用户权限管理** — 管理员/普通用户角色，主播账号+授权房间，公网部署更安全
- 📊 **弹幕记录** — 完整历史弹幕查看、日期日历筛选、多选批量导出CSV、排序切换
- ⚙️ **配置导入/导出** — 机器人配置和 AI 配置均可导出 JSON 分享、导入恢复
- 🔄 **进程管理重构** — 多房间独立进程管理，退出时自动清理所有 Bot 进程
- 🖥️ **启动脚本** — Windows `start.bat`（支持 Ctrl+C 一键退出），Linux/macOS `start.sh`
- 🐳 **Docker 多架构** — AMD64 + ARM64 镜像，树莓派/苹果 M 系列完美支持
- 📦 **单源版本号** — 改 `VERSION` 一个文件，Python/WebUI/构建自动同步
